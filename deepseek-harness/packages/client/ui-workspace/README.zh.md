...
- **浏览器内没有 Session 或 Workspace 的取消归档入口**：会话和工作区都可以归档（工作区通过行菜单），但浏览器不提供二者的查看或取消归档控件；恢复属于 Host RPC 领域（桌面恢复面板），且工作区归档绝不会删除 Session 或注册记录。
