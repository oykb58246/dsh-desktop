...
- `ctx.workspaceRegistry.archiveSession(id)` / `archivedSessionIds` — the registry-global session archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so a future unarchive restores its position. Archiving accepts any live or persisted session (accounted or Ungrouped), resolves without writing for an already archived id, and rejects an unknown id. `ctx.workspaceRegistry.unarchiveSession(id)` removes an id from the set idempotently (unknown ids are write-free no-ops). State written before the field existed parses with an empty set.
- `ctx.workspaceRegistry.archiveWorkspace(id)` / `unarchiveWorkspace(id)` / `archivedWorkspaceIds` — the registry-global workspace archive set: an archived workspace keeps its record, session account, and durable-order position while grouping surfaces hide the whole group, so a future unarchive restores it as-is. `archiveWorkspace` resolves without writing for an already archived id and rejects an unknown id with `WorkspaceUnknownWorkspaceError`; `unarchiveWorkspace` is an idempotent write-free no-op for unknown or already unarchived ids. Workspace archiving never moves sessions into the session archive set — the workspace flag hides the group whole, and session-level archive flags stay independent. State written before the field existed parses with an empty set.
