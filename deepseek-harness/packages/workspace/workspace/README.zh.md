...
- `ctx.workspaceRegistry.archiveSession(id)`/`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合：被归档的会话从各分组视图中消失，但其会话日志和 `sessionIds` 席位保持不变，未来取消归档时可恢复原位置。归档接受任何实时或已持久化的会话（无论已记账还是 Ungrouped），对已归档的 id 直接完成而不写入，并拒绝未知 id。`ctx.workspaceRegistry.unarchiveSession(id)` 幂等地从集合中移除一个 id（未知 id 为不写入的 no-op）。在该字段出现之前写入的状态解析为一个空集合。
- `ctx.workspaceRegistry.archiveWorkspace(id)`/`unarchiveWorkspace(id)`/`archivedWorkspaceIds`：注册表级全局工作区归档集合：被归档的工作区保留其记录、会话记账与持久化顺序位置，仅由分组视图整体隐藏，未来取消归档时原样恢复。`archiveWorkspace` 对已归档的 id 直接完成而不写入，并拒绝未知 id（抛 `WorkspaceUnknownWorkspaceError`）；`unarchiveWorkspace` 对未知或已取消归档的 id 为幂等的不写入 no-op。工作区归档不会把会话移入会话归档集合——工作区标志整体隐藏该组，会话级归档标志保持独立。在该字段出现之前写入的状态解析为一个空集合。
